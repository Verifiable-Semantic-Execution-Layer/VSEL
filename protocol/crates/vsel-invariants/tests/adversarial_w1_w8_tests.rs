...
fn w1_1_negative_balance_total_supply_mismatch() {
    ...
    assert!(
        !result.valid,
        "W1.1: Mismatched total_supply must be rejected by G_valid"
    );
}
...
fn w1_1_balance_sum_exceeds_total_supply() {
    ...
    assert!(
        !result.valid,
        "W1.1: Balance sum > total_supply must be rejected by G_struct"
    );
}
...
fn w1_2_inconsistent_derived_aggregates() {
    ...
    s.derived
        .aggregates
        .insert("total_balance".to_string(), 9999);
    assert!(
        !valid_state(&s),
        "W1.2: valid_state must reject inconsistent derived aggregates"
    );
}
...
fn w1_4_metadata_regression_nonzero_commitment_at_genesis() {
    ...
    assert!(
        !result.valid,
        "W1.4: Genesis with non-zero commitment must be rejected by G_mono"
    );
}
...
fn w1_4_metadata_regression_zero_commitment_nongenesis() {
    ...
    assert!(
        !result.valid,
        "W1.4: Non-genesis with zero commitment must be rejected by G_mono"
    );
}
...
fn w1_5_unreachable_state_apply_produces_different_result() {
    ...
    fake_post
        ...
        .system_data
        .parameters
        .insert("rogue_param".to_string(), vec![0xDE, 0xAD]);
...
fn w2_2_hidden_mutation_environment_changed() {
    ...
    assert!(
        !result.valid,
        "W2.2: Hidden environment mutation must be rejected by L_valid"
    );
}
...
fn w2_3_resource_destruction_balance_vanishes() {
    ...
    assert!(
        !result.valid,
        "W2.3: Resource destruction must be rejected by L_cons"
    );
}
...
fn w2_4_unauthorized_empty_classical_sig() {
    ...
    assert!(
        result.is_err(),
        "W2.4: Empty classical_sig must be rejected"
    );
    assert!(matches!(
        result.unwrap_err(),
...
fn w2_5_precondition_violation_transfer_nonexistent_sender() {
    ...
    assert!(
        result.is_err(),
        "W2.5: Transfer from non-existent sender must be rejected"
    );
    assert!(matches!(
        result.unwrap_err(),
...
fn w2_5_precondition_violation_fabricated_transfer() {
    ...
    assert!(
        !result.valid,
        "W2.5: Fabricated transfer exceeding balance must be rejected by L_cons"
    );
}
...
fn w3_1_broken_chain_hash_tampered_entry() {
    ...
    assert!(
        !verify_trace(&trace),
        "W3.1: Tampered chain hash must be rejected"
    );
}
...
fn w3_1_broken_chain_hash_tampered_commitment() {
    ...
    assert!(
        !verify_trace(&trace),
        "W3.1: Tampered final commitment must be rejected"
    );
}
...
fn w3_2_missing_transition_gap_in_indices() {
    ...
    assert!(
        !verify_trace(&trace),
        "W3.2: Missing entry (index gap) must be rejected"
    );
}
...
fn w3_2_missing_transition_state_chain_broken() {
    ...
    assert!(
        !verify_trace(&trace),
        "W3.2: Broken state commitment chain must be rejected"
    );
}
...
fn w3_3_reordered_entries() {
    ...
    assert!(
        !verify_trace(&trace),
        "W3.3: Reordered entries must be rejected"
    );
}
...
fn w3_3_duplicate_entries() {
    ...
    assert!(
        !verify_trace(&trace),
        "W3.3: Duplicate entries must be rejected"
    );
}
...
fn w4_1_fabricated_observable_wrong_outputs() {
    ...
    let fake_obs = Observable {
        ...
        outputs: vec![OutputEvent {
            event_type: "fabricated_event".to_string(),
            data: vec![0xDE, 0xAD],
        }],
    ...
    assert_ne!(
        fake_obs, real_obs,
        "W4.1: Fabricated observable must differ from real"
    );
    let rederived = obs(&s, &sigma, &s_prime);
    assert_eq!(
        rederived, real_obs,
        "W4.1: Observable must be deterministically derivable"
    );
    assert_ne!(
        rederived, fake_obs,
...
fn w4_1_fabricated_observable_wrong_gas() {
    ...
    assert_ne!(
        fake_obs, real_obs,
        "W4.1: Observable with fabricated gas must differ"
    );
}
...
fn w4_2_missing_observable_empty_outputs() {
    ...
    assert!(
        ...
        "Transfer should produce output events"
    );
...
fn w4_3_noop_with_non_null_observable() {
    ...
    assert!(
        real_obs.outputs.is_empty(),
        "Noop should produce no output events"
    );
    let fake_obs = Observable {
        transition_class: TransitionClass::Noop,
        outputs: vec![OutputEvent {
            event_type: "phantom_event".to_string(),
            data: vec![0xFF],
        }],
        gas_used: real_obs.gas_used,
    ...
    assert_ne!(
        rederived, fake_obs,
        "W4.3: Noop with fabricated outputs must be detectable"
    );
}
...
fn w5_1_wrong_payload_empty_classical_key() {
    ...
    assert!(
        result.is_err(),
        "W5.1: Empty classical public key must be rejected"
    );
    assert!(matches!(
        result.unwrap_err(),
        ExecutionError::MalformedInput(_)
...
fn w5_1_wrong_payload_empty_pqc_key() {
    ...
    assert!(
        result.is_err(),
        "W5.1: Empty PQC public key must be rejected"
    );
}
...
fn w5_2_replayed_authorization_same_nonce() {
    ...
    assert!(
        r2.is_ok(),
        "Engine accepts replayed auth — replay prevention is at trace/proof level"
    );
}
...
fn w5_3_cross_domain_mismatched_domain() {
    ...
    assert!(
        result.is_ok(),
        "Engine accepts mismatched domain — domain matching is at verification level"
    );
}
...
fn w6_1_reordered_batch_sequential_equivalence() {
    ...
    let s2 = apply(&s1, &d2);
    assert_eq!(
        batch.post_state.canonical, s2.canonical,
...
fn w6_2_skipping_validation_halts_on_invalid() {
    ...
    assert!(
        result.is_err(),
        "W6.2: Batch must halt on invalid intermediate input"
    );
    assert!(matches!(
        result.unwrap_err(),
        ExecutionError::MalformedInput(_)
...
fn w6_3_phantom_operations_extra_deposit() {
    ...
    assert!(batch_with_phantom
        .post_state
        .canonical
        .accounts
        .contains_key(&AccountId([99u8; 32])));
    assert_eq!(
        batch_with_phantom
            .post_state
            .canonical
...
fn w6_3_phantom_operations_detectable_via_trace() {
    ...
    assert_eq!(
        batch.intermediate_results.len(),
        2,
        "W6.3: Batch with 2 inputs must have exactly 2 intermediate results"
    );
}
...
fn w7_1_wrong_pre_state_commitment() {
    ...
    assert!(
        !verify_trace(&trace),
        "W7.1: Wrong pre_state_commitment must be rejected"
    );
}
...
fn w7_1_wrong_post_state_commitment() {
    ...
    assert!(
        !verify_trace(&trace),
        "W7.1: Wrong post_state_commitment must be rejected"
    );
}
...
fn w7_1_state_commitment_mismatch_with_actual_state() {
    ...
    assert_ne!(
        h1, h2,
        "W7.1: Different canonical states must produce different commitments"
    );
}
...
fn w7_2_wrong_chain_hash_entry() {
    ...
    assert!(
        !verify_trace(&trace),
        "W7.2: Wrong chain hash must be rejected"
    );
}
...
fn w7_2_wrong_chain_hash_swapped() {
    ...
    assert!(
        !verify_trace(&trace),
        "W7.2: Swapped chain hashes must be rejected"
    );
}
...
fn w7_2_verify_chain_rejects_tampered_hashes() {
    ...
    assert!(
        !verify_chain(&[e1.clone(), e2.clone()], &[Hash([0xFFu8; 32]), h2.clone()]),
        "W7.2: verify_chain must reject tampered first hash"
    );
    assert!(
        !verify_chain(&[e1, e2], &[h1, Hash([0xFFu8; 32])]),
...
fn w8_1_inconsistent_shared_state_different_balances() {
    ...
    assert!(
        total_a + total_b > balance_a.max(balance_b),
...
fn w8_1_inconsistent_shared_state_different_nonces() {
    ...
    if let Some(acc) = c_a.accounts.get_mut(&AccountId([1u8; 32])) {
        acc.nonce = 10;
    }
    let s_a = build_state_at_seq(c_a, 1);
    let mut c_b = canonical_with_account([1u8; 32], 1000);
    if let Some(acc) = c_b.accounts.get_mut(&AccountId([1u8; 32])) {
        acc.nonce = 5;
    }
    let s_b = build_state_at_seq(c_b, 1);
    ...
    assert_ne!(
        nonce_a, nonce_b,
        "W8.1: Systems have inconsistent nonces for shared account"
    );
}
...
fn w8_2_resource_creation_cross_system() {
    ...
    assert_ne!(
        ...
        "W8.2: Cross-system total supply changed"
    );
    assert!(
        total_post > total_pre,
        "W8.2: Resources were created ({} -> {})",
        total_pre,
        total_post
    );
}
